...
export class BaseService implements IBaseService {
    ...
    public async getExtPagedListItems(queryurl: string): Promise<any> {
        let collection: any = []
        for await (const items of this.sitesp.web.getList(queryurl).items.select('*').top(250) as any) {
            collection = collection.concat(items)
        }
        return collection
    }

    public getItemsExtSelectFilterExpand(queryurl: string, select: string, filter: string, expand: string): Promise<any> {
    ...
    public getItemsExtSelectFilter(queryurl: string, select: string, filter: string): Promise<any> {
        return this.sitesp.web.getList(queryurl).items
            .select(select)
            .filter(filter)()
    }

    public getItemsExtFilter(queryurl: string, filter: string): Promise<any> {
        return this.sitesp.web.getList(queryurl).items.filter(filter)()
    }
    ...
    public getExtItemsSelect(queryurl: string, select: string): Promise<any> {
        return this.sitesp.web.getList(queryurl).items
            .select(select)()
    }
    ...
    public getExtItemsFilter(queryurl: string, filter: string): Promise<any> {
        return this.sitesp.web.getList(queryurl).items
            .filter(filter)()
    }
    ...
    public getItemSelectExpandOrderBy(queryurl: string, select: string, expand: string, Orderby: string): Promise<any> {
        return this.sp.web.getList(queryurl).items
            .select(select)
            .expand(expand)
    ...
    public getExtListItems(url: string): Promise<any> {
        return this.sitesp.web.getList(url).items();
    }
