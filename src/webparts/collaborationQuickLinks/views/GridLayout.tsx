...
interface IGridLayoutProps {
  ...
  iconColor: string;
  cardColor: string;
  cardAlignment?: 'left' | 'center' | 'right'; // Optional alignment prop
  cardHoverColor?: string; // Optional hover color prop
}
...
const GridLayout: React.FC<IGridLayoutProps> = ({
  ...
  const tileWidth = settings.tileSize?.tileWidth || 200;
  const tileHeight = settings.tileSize?.tileHeight || 150;
  ...
  return (
    <div className={`${styles.gridContainer} ${styles[cardAlignment || 'center']}`} style={containerStyle}>
      {linkItems.map((link, index) => (
        ...
        >
          <TooltipHost
            content={
              <div style={{ lineHeight: '1.5', color: textColor || '#ffffff' }}>
                <div>
          ...
          >
            ...
            >
              <div className={styles.cardContent}>
                {link.Icon ? (
                  link.Icon.startsWith('http') ? (
                    <img
                      ...
                      style={{ ...getOpacityStyle(link), ...getImageStyle(),  ...getBorderRadiusStyle(link),}}
                    />
                  ) : (
                    <Icon
                ...
                <div className={styles.cardTitle} title={link.Title}>
