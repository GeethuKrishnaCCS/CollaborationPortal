...
import styles from './CollaborationDirectory.module.scss';
import { useEffect, useState } from 'react';
import type { ICollaborationDirectoryProps } from './ICollaborationDirectoryProps';

import { PersonaCard } from "./PersonaCard/PersonaCard";
import { spservices } from "../../../SPServices/spservices";
import { ICollaborationDirectoryState } from "./ICollaborationDirectoryState";
...
const CollaborationDirectory: React.FC<ICollaborationDirectoryProps> = (props) => {
  ...
  const [state, setstate] = useState<ICollaborationDirectoryState>({
    users: [],
    isLoading: true,
    errorMessage: "",
    hasError: false,
 ...
 const _onPageUpdate = async (pageno?: number): Promise<void> => {
  ...
  const filterUsers = (users: any[]) => {
    ...
    return users.filter((user: { Department: any; JobTitle: any; WorkPhone: any; WorkEmail: any; BaseOfficeLocation: any; }) => {
      const filterSettings = props.filterSettings;
       ...
       if (filterSettings.filterDepartment) {
      if (!user.Department || 
          !user.Department.toLowerCase().includes(filterSettings.filterDepartment.toLowerCase())) {
        return false;
      }
    }
  ...
  const _searchByAlphabets = async (initialSearch: boolean): Promise<void> => {
    ...
    logUniqueDepartments(filteredUsers);
    setstate({
      ...state,
      searchText: '',
  ...
  const logUniqueDepartments = (users: any[]) => {
 ...
 const _searchUsers = async (searchText: string): Promise<void> => {
      ...
      if (searchText.length > 0) {
        ...
        setstate({
          ...
          hasError: false
            ...
            console.log("🔍 All Users (Initial Load):", users?.PrimarySearchResults);
        setalphaKey('0');
      } else {
  ...
  const _debouncesearchUsers = debounce(100, _searchUsers);
  ...
  const sortUsers = (users: any[], order: "asc" | "desc", field: keyof typeof users[0]) => {
...
const handleSortToggle = () => {
  ...
  const sortedUsers = sortUsers(state.users, newOrder, "Title"); // or "Department"
  setstate(prev => ({
    ...prev,
    users: sortedUsers
  }));
  ...
  {/* Combined search and dropdown container */}
  ...
  onChange={(ev, value) => value && _sortPeople(value.key.toString())}
  styles={{ 
    // root: { marginLeft: 10 }, // Add spacing from the left
    dropdown: { 
      width: 100,
      selectors: {
        '.ms-Dropdown-title': {
          borderRadius: 20, 
          backgroundColor:' #C464EC'
,         color: 'white',
          borderStyle: 'none',
          borderColor:'none'
        }
...
<span onClick={handleSortToggle} style={{ fontSize: '20px' }}>&#8645;</span>
</div>
  </div>
            ...
            <>
              ...
              <div>
                <Paging
                  ...
                  onPageUpdate={(page) => _onPageUpdate(page)}
